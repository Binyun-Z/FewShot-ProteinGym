...
from data_utils import Mutation_Set
...
def train(model, model_reg, trainloder, optimizer, tokenizer, lambda_reg,pbar):
...
def main():
    ...
    parser.add_argument('--dataset', type=str,default='substitutions_singles',help='the dataset name')
    ...
    parser.add_argument('--fold_spilt_type', type=str, default='fold_random_5', help='method for split data')
    ...
    with accelerator.main_process_first():
        ...
        for i in range(5):
            if i == 4: #后面改为交叉验证
                val_csv = pd.read_csv(f'data/cleaned_split_data_singles/{args.fold_spilt_type}/data_{i}.csv')   #using 1/5 train data as validation set
            temp_csv = pd.read_csv(f'data/cleaned_split_data_singles/{args.fold_spilt_type}/data_{i}.csv')
            train_csv = pd.concat([train_csv, temp_csv], axis=0)
        test_csv = val_csv 
        # train_csv = pd.read_csv(f'data/test_model_data/{args.fold_spilt_type}/data_0.csv')
        # test_csv = pd.read_csv(f'data/test_model_data/{args.fold_spilt_type}/data_1.csv')
        # val_csv = pd.read_csv(f'data/test_model_data/{args.fold_spilt_type}/data_2.csv')
    ...
    for epoch in range(int(config['max_epochs'])):
        ...
        if best_sr > sr:
            ...
            save_path = os.path.join('checkpoint', f'{dataset}',
                                     f'seed{args.model_seed}_all_data_0726_1300')
